...
static void tcp_tw_schedule(struct tcp_tw_bucket *tw, int timeo)
{
	 ...
	 *
	 * 基本原理：如果FIN到达并且我们进入了TIME-WAIT状态，我们确认FIN的ACK可能会丢失。
	 * 如果N个后续重传的FIN（或先前的段）丢失（此类事件的概率为p^(N+1)，其中p是丢失单个数据包的概率，
	 * 检测丢失的时间约为RTO(2^N-1)指数退避）。
	 * 正常时间等待长度是这样计算的，我们至少等待一个重传的FIN（最大RTO为120秒）。
	 * [顺便说一句，Linux。在BSD之后，违反了这个要求只等待60秒，
	 * 我们应该至少等待240秒。嗯，240消耗资源太多了8)]
	 * 该间隔不会减少以捕获旧的重复项和对我们为两个 MSL 生存的游荡段的响应。
	 * 但是，如果我们使用PAWS来检测旧的重复项，我们可以将间隔减少到RTO所需的界限，而不是MSL。
	 * 因此，如果peer理解PAWS，我们会在3.5RTO之后杀死tw个bucket（重要的是这个数字大于 TS 滴答！）
	 * 并在PAWS的帮助下检测旧的重复项。
	 */
